...
public class PromotionHelper {
	...
	public static List<List<Type>> getBinaryPromotionLists(List<Type> originalSignature, int i, int j) {
	...
	public static boolean typeAIsPredecessorTypeOrSame(PrimitiveType a, PrimitiveType b){
		if(a == PrimitiveType.CHARACTER) {
			return true;
		}else if (a == PrimitiveType.INTEGER) {
			return  b == PrimitiveType.INTEGER ||
					b == PrimitiveType.RATIONAL ||
	...
	public static boolean listAIsPredecessorListOrSame(List<Type> a, List<Type> b){
		for(int i = 0; i < a.size(); i++) {
			if(!typeAIsPredecessorTypeOrSame((PrimitiveType)a.get(i),(PrimitiveType)b.get(i)))
				return false;
		}
		return true;
	}
